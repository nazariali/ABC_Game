...
def main():
    ...
    def save_clicked():
        ...
        messagebox.showinfo('ABC Games', 'Game is saved to File')

    # ---------------------------------------------------------------------
    def load_clicked():
        global grid, merged, random_flag, pushed, current_score, game_over, win
        if messagebox.askyesno('ABC Games', 'You are going to load previous saved game.\nAre you sure?'):
            grid_temp = file_to_list_4dim("files/save.txt")
            if not grid_temp:
                messagebox.showinfo('ABC Games', 'Have not saved game before')
            else:
                grid = grid_temp
                ...
                result_label.configure(text="Use Arrow Keys to mov")
                grid_display()
                score_display()
    ...
    # ---------------------------------------------------------------------
    def new_clicked():
        global  current_score, game_over, win, pushed, random_flag, merged, best_score
        if messagebox.askyesno('ABC Games', 'A new game is starting.\nAre you sure?'):
            ...
            for _ in range(2):
                add_random_cell()
            h_score = read_file("files/highscore.txt")
            if h_score:
                best_score = int(h_score)
            else:
                best_score = 0
            grid_display()
            score_display()
